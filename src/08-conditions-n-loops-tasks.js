...
function doRectanglesOverlap(rect1, rect2) {
  ...
  const a1 = rect2.left;
  const a2 = rect2.left + rect2.width;
  const b1 = rect2.top;
...
function isInsideCircle(circle, point) {
  ...
  const xo = circle.center.x;
  const yo = circle.center.y;
  const xp = point.x;
  const yp = point.y;

  return r > Math.sqrt((xo - xp) ** 2 + (yo - yp) ** 2);
}
...
class Stack {
  ...
  top() {
    return this.parr[this.pcount] || false;
  }
}
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 1; i < pathes.length; i += 1) {
    for (let j = 0; j <= tmp.lastIndexOf('/'); j += 1) {
      if (tmp[j] === pathes[i][j]) {
        path += tmp[j];
      } else {
        tmp = path;
